...
from timetables.models import GodzinyLekcyjne, Zajecia, DniTygodnia, PlanWpis, PlanyZajec, Wydarzenie
...
POSSIBLE_GRADE_VALUES = [
    Decimal('1.00'), Decimal('1.75'), Decimal('2.00'), Decimal('2.75'),
    Decimal('3.00'), Decimal('3.75'), Decimal('4.00'), Decimal('4.75'),
    Decimal('5.00'), Decimal('5.75'), Decimal('6.00')
]
WAGA_OCEN = [1, 2, 3, 4, 5]
...
def populate_przedmioty():
    ...
    przedmioty_data = [
        'Trening Mentalny', 'WF', 'Język Angielski', 'Biznes i Zarządzanie', 'Matematyka', 'Fizyka',
        'Informatyka', 'Biologia', 'Historia', 'Grafika Komputerowa (Specjalizacja)', 'Język Polski',
        'Język Hiszpański', 'Chemia', 'Geografia', 'Religia', 'IT (Specjalizacja)'
    ]
    created = []
    for nazwa in przedmioty_data:
        ...
        p, created_flag = Przedmiot.objects.get_or_create(nazwa=clean_nazwa,
                                                          defaults={'nazwa_skrocona': skrot,
                                                                    'numer': numer_val,
...
def populate_oceny_i_obecnosci(uczniowie_qs, nauczyciele_qs, przedmioty_qs):
    ...
    if not (uczniowie_qs.exists() and nauczyciele_qs.exists() and przedmioty_qs.exists()):
    ...
    if GodzinyLekcyjne.objects.count() == 0:
        ...
        for num, od, do, tr in default_hours:
            GodzinyLekcyjne.objects.create(Numer=num, CzasOd=od, CzasDo=do, CzasTrwania=tr)
    ...
    for p in przedmioty_qs.iterator():
        przedmiot_nauczyciele_map[p.pk] = list(p.nauczyciele.values_list('pk', flat=True))
    ...
    for uczen in uczniowie_qs.iterator():
        ...
        for _ in range((num_school_days // 30)):
            ...
            for _ in range(random.randint(1, MAX_OBECNOSCI_PER_UCZEN_PER_DAY)):
                ...
                frek = Frekwencja(Data=day, uczen_id=uczen.pk, godzina_lekcyjna_id=(gl.pk if gl is not None else None), status_id=(status.pk if status is not None else None))
    ...
    for uczen in uczniowie_qs.iterator():
        sampled_przedmioty = random.sample(list(przedmioty_qs), k=min(3, max(1, len(list(przedmioty_qs)))))
...
def populate_punkty_zachowania(uczniowie_qs, nauczyciele_qs):
    ...
    for uczen in uczniowie_qs.iterator():
        for _ in range(random.randint(1, MAX_PUNKTY_ZACHOWANIA_PER_UCZEN)):
            ...
            nauczyciel_pk = random.choice(nauczyciele_list) if nauczyciele_list else None
            zp = ZachowaniePunkty(uczen_id=uczen.pk, punkty=punkty, opis=opis, nauczyciel_wpisujacy_id=nauczyciel_pk)
            to_create.append(zp)
...
def ensure_godziny_i_dni():
    ...
    if DniTygodnia.objects.count() == 0:
        days = [('Poniedziałek',1), ('Wtorek',2), ('Środa',3), ('Czwartek',4), ('Piątek',5)]
        for name, num in days:
            DniTygodnia.objects.create(Nazwa=name, Numer=num)
    print(f"  Ensured GodzinyLekcyjne ({GodzinyLekcyjne.objects.count()}) and DniTygodnia ({DniTygodnia.objects.count()})")
...
def create_zajecia_and_plans():
    ...
    if not przedmioty or not nauczyciele or not klasy or not godziny or not dni:
        print("  Missing prerequisites (przedmioty/nauczyciele/klasy/godziny/dni). Skipping plans/zajecia.")
        return
    ...
    for kl in klasy:
        ...
        plan, _ = PlanyZajec.objects.get_or_create(klasa=kl, ObowiazujeOdDnia=plan_start)
        ...
        for _ in range(num_wpisow):
            ...
            wpis = PlanWpis.objects.create(godzina_lekcyjna=godz, dzien_tygodnia=dzien, zajecia=zaj)
            plan.wpisy.add(wpis)
    print(f"  Created/ensured {len(zajecia_instances)} Zajecia and plany for {len(klasy)} klas")
...
def create_wydarzenia_tematy_prace():
    ...
    for _ in range(len(klasy) * 3):
        ...
        Wydarzenie.objects.create(tytul=t, opis=opis, data=data, klasa=kl, przedmiot=pr, nauczyciel=nauc)
    ...
    for pr in przedmioty:
        for _ in range(random.randint(1,3)):
            ...
            czas_do = (timezone.datetime.combine(date.today(), czas_od) + timedelta(minutes=czas_realizacji)).time()
            nauc = random.choice(nauczyciele)
            Temat.objects.create(tresc=tresc, data=data, numer_lekcji=numer_lekcji, czas_realizacji=czas_realizacji, czas_od=czas_od, czas_do=czas_do, przedmiot=pr, nauczyciel=nauc)
            tematy_created += 1
    ...
    for pr in przedmioty:
        for kl in random.sample(klasy_cycle, k=min(3, len(klasy_cycle))):
            ...
            PracaDomowa.objects.create(klasa=kl, przedmiot=pr, nauczyciel=nauc, opis=opis, termin=termin)
            prace_created += 1

    print(f"  Created {wydarzenia_created} wydarzenia, {tematy_created} tematy, {prace_created} prace domowe")
...
def create_rodzice_for_uczniowie(sample_fraction=0.15):
    ...
    for uc in random.sample(uczniowie, k=max(1, int(len(uczniowie) * sample_fraction))):
        ...
        user = User.objects.create(username=username, email=email, password=HASHED_PASSWORD, first_name=first, last_name=last)
        user.is_active = True
        user.save()
        rodzic = Rodzic.objects.create(user=user, telefon=normalize_phone(fake.phone_number()))
