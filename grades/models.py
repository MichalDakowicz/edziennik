...
class Ocena(models.Model):
    ...
    nauczyciel = models.ForeignKey(
        ...
        related_name="wystawione_oceny",
        null=True,
        blank=True,
    )

    przedmiot = models.ForeignKey(
        Przedmiot, on_delete=models.CASCADE, related_name="oceny"
    )
...
class OcenaOkresowa(models.Model):
    uczen = models.ForeignKey(
        Uczen, on_delete=models.CASCADE, related_name="oceny_okresowe"
    )
    ...
    przedmiot = models.ForeignKey(
        Przedmiot,
        on_delete=models.CASCADE,
        related_name="oceny_okresowe",
        null=True,
        blank=True,
    )

    nauczyciel = models.ForeignKey(
        Nauczyciel, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class OcenaKoncowa(models.Model):
    uczen = models.ForeignKey(
        Uczen, on_delete=models.CASCADE, related_name="oceny_koncowe"
    )

    wartosc = models.DecimalField(max_digits=3, decimal_places=2)

    przedmiot = models.ForeignKey(
        Przedmiot, on_delete=models.CASCADE, related_name="oceny_koncowe"
    )
...
class ZachowaniePunkty(models.Model):
    uczen = models.ForeignKey(
        Uczen, on_delete=models.CASCADE, related_name="punkty_zachowania"
    )
    punkty = models.IntegerField()
    opis = models.TextField(blank=True, null=True)
    data_wpisu = models.DateTimeField(auto_now_add=True)
    nauczyciel_wpisujacy = models.ForeignKey(
        Nauczyciel,
